...
datatype geom_exp = 
	 ...
	 | Var of string
         | Shift of real * real * geom_exp (* dX, dY and subexpression to shift *)
...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(dx,dy,e1) => let val to_shift = eval_prog(e1, env)
                           in
                               case to_shift of
                                   NoPoints => to_shift
    ...
    case exp of
      ...
      | LineSegment(x1,y1,x2,y2) =>
        ...
        then Point (x1,y1)
            ...
            if real_close (x1,x2)
            then (if y1 < y2 then LineSegment(x1,y1,x1,y2) else LineSegment(x1,y2,x1,y1))
            else if (x2 < x1)
